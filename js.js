...
start.addEventListener('click',() => {
    ...
    let lerp = (a, b, t) => 2 * a + (2 * b - 2 * a) * (1 - Math.cos(t * Math.PI)) / 2;
    let noise = x => {
        x = x * 0.005 % 255;
    ...
    let player = new function() {
        ...
        this.img.src = './1.png';
        this.draw = function() {
            let p1 = c.height - noise(t + this.x) * 0.55; 
            let p2 = c.height - noise(t + 3 + this.x) * 0.55; 
            let grounded = 0;
    ...
    function loop() {
        ...
        for (let i = 0; i < c.width; i++) {
            ctx.lineTo (i, c.height - noise(t + i) * 0.55);
        }
